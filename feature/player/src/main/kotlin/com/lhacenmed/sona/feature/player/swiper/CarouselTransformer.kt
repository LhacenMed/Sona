...
/**
 * Lays the queue's covers out as a Material 3 carousel: every page pinned in place and masked, so a
 * swipe shrinks the outgoing cover towards one edge while the incoming one grows in from the other,
 * a gap opening between them and each cover's content drifting with its mask. Auxio's
 * `CarouselTransformer`, copied as is.
 */
internal class CarouselTransformer : ViewPager2.PageTransformer {
